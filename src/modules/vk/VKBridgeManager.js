...
export class VKBridgeManager {
    ...
    async showOrderBox() {
        ...
        try {
            ...
            if (userInfo && userInfo.id) {
                ...
                try {
                    const response = await fetch(`http://localhost:3000/api/vk/premium-status/${userInfo.id}`);
    ...
    async updateUserInBackend(userInfo) {
        try {
            const response = await fetch('http://localhost:3000/api/vk/user', {
                method: 'POST',
                headers: {
    ...
    /**
     * Manually activate premium for current user
     */
    async manuallyActivatePremium() {
        try {
            const userInfo = await this.getUserInfo();
            if (userInfo && userInfo.id) {
                const response = await fetch(`http://localhost:3000/api/vk/activate-premium/${userInfo.id}`, {
                    method: 'POST'
                });
                ...
                if (result.success) {
                    await this.showNotification('Премиум доступ активирован!');
                    this.trackVKEvent('vk_premium_manually_activated', {
                        user_id: userInfo.id
                    });
                    return { success: true, premiumActivated: true };
                }
            }
            return { success: false, error: 'No user info or activation failed' };
        } catch (error) {
            console.error('Error manually activating premium:', error);
            return { success: false, error };
        }
    }
