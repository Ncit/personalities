...
export class VKBridgeManager {
    ...
    /**
     * Purchase VK Premium subscription
     */
    async purchasePremium() {
        this.trackVKEvent('vk_premium_purchase_attempted', {
            bridge_available: !!this.bridge,
            vk_platform: this.isVKPlatform,
            user_id: this.userInfo?.id || null
        });
        ...
        if (!this.isVKFeatureSupported('payment')) {
            console.log('VK payment not available - not in VK environment');
            
            this.trackVKEvent('vk_premium_purchase_not_available', {
                reason: 'not_vk_environment'
            });
            ...
            return {
                success: false,
                error: 'not_available',
                message: 'Premium purchase is only available in VK environment'
            };
        }
        
        try {
            // Define premium product
            const premiumProduct = {
                ...
                price: 40, // VK votes
                currency: 'votes',
                image_url: 'https://your-domain.com/premium-icon.png'
            };
            
            // Track purchase initiation
            this.trackVKEvent('vk_premium_purchase_initiated', {
                product_id: premiumProduct.product_id,
                price: premiumProduct.price,
                currency: premiumProduct.currency
            });
            
            // Open VK payment form
            const result = await this.bridge.send('VKWebAppOpenPayForm', {
                app_id: 0, // Replace with your VK app ID
                user_id: this.userInfo?.id,
                receiver_id: 0, // Replace with your community ID
                amount: premiumProduct.price,
                description: premiumProduct.description,
                currency: premiumProduct.currency,
                test_mode: false // Set to true for testing
            });
            
            console.log('VK payment result:', result);
            
            // Track payment result
            this.trackVKEvent('vk_premium_payment_result', {
            ...
            if (result.status === 'success') {
                // Payment successful
                this.trackVKEvent('vk_premium_purchase_success', {
                    transaction_id: result.transaction_id,
                    amount: result.amount,
                    currency: result.currency
                });
                ...
                this.trackVKEvent('vk_premium_purchase_failed', {
                    status: result.status,
            ...
            // Handle VK error gracefully
            const errorResult = this.handleVKError(error, 'purchasePremium');
            
            this.trackVKEvent('vk_premium_purchase_error', {
                error_type: error.error_type,
                error_code: error.error_data?.error_code,
                error_reason: error.error_data?.error_reason,
                error_message: error.message
            ...
            return {
                success: false,
                error: 'vk_error',
                message: error.error_data?.error_reason || error.message || 'Premium purchase failed',
    ...
    async checkPremiumStatus() {
        this.trackVKEvent('vk_premium_status_check_attempted', {
            bridge_available: !!this.bridge,
            vk_platform: this.isVKPlatform,
            user_id: this.userInfo?.id || null
        });
        ...
        if (!this.isVKFeatureSupported('payment')) {
            console.log('VK payment status check not available - not in VK environment');
            
            this.trackVKEvent('vk_premium_status_check_not_available', {
                reason: 'not_vk_environment'
            });
            ...
            return {
                success: false,
                error: 'not_available',
                message: 'Premium status check is only available in VK environment'
            };
        }
        
        try {
            // Check subscription status via VK API
            const result = await this.bridge.send('VKWebAppGetUserInfo');
            
            // For now, we'll check local storage for premium status
            // In a real implementation, you'd verify with your backend
            ...
            this.trackVKEvent('vk_premium_status_checked', {
                is_premium: isPremium,
                expiry_date: premiumExpiry
            });
            ...
            return {
                ...
                message: isPremium ? 'Premium subscription active' : 'No active premium subscription'
            };
     ...
     */
    async restorePremiumPurchase() {
        this.trackVKEvent('vk_premium_restore_attempted', {
            bridge_available: !!this.bridge,
            vk_platform: this.isVKPlatform,
            user_id: this.userInfo?.id || null
        });
        ...
        if (!this.isVKFeatureSupported('payment')) {
            console.log('VK payment restore not available - not in VK environment');
            
            this.trackVKEvent('vk_premium_restore_not_available', {
                reason: 'not_vk_environment'
            });
            ...
            return {
                success: false,
                error: 'not_available',
                message: 'Premium restore is only available in VK environment'
            };
        }
        ...
        try {
            ...
            if (premiumStatus.success && premiumStatus.isPremium) {
                this.trackVKEvent('vk_premium_restore_success', {
                ...
                this.trackVKEvent('vk_premium_restore_no_purchase', {
            ...
            console.error('Error restoring premium purchase:', error);
            
            this.trackVKEvent('vk_premium_restore_error', {
                error_message: error.message
            });
            ...
            return {
                ...
                message: 'Failed to restore premium purchase'
            };
        }
    }

    /**
     * Get premium product information
     */
